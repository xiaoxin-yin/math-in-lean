...
theorem ex1 (φ : G →* H) (S T : Subgroup H) (hST : S ≤ T) : comap φ S ≤ comap φ T := by
  intro x hx
  rw [mem_comap] at * -- Lean does not need this line
  exact hST hx

theorem ex2 (φ : G →* H) (S T : Subgroup G) (hST : S ≤ T) : map φ S ≤ map φ T := by
  intro x hx
...
theorem ex3 (φ : G →* H) (ψ : H →* K) (U : Subgroup K) :
...
theorem ex4 (φ : G →* H) (ψ : H →* K) (S : Subgroup G) :
